...
import {
  LayoutDashboard, AlertTriangle, Users, Layers, FileWarning, FileBarChart, ChevronLeft, ChevronRight,
} from 'lucide-react'

const NAV = [
  { to: '/', icon: LayoutDashboard, label: 'Command', badge: null, end: true },
  { to: '/exceptions', icon: AlertTriangle, label: 'Exceptions', badge: 'exceptions' },
  { to: '/patients', icon: Users, label: 'Registry', badge: null },
  { to: '/cohorts', icon: Layers, label: 'Cohorts', badge: null },
  { to: '/discrepancies', icon: FileWarning, label: 'Data quality', badge: 'disc' },
  { to: '/reports', icon: FileBarChart, label: 'Reports', badge: null },
]

export default function Layout() {
  const { stats, discrepancies, notifications } = useRegistry()
  const [collapsed, setCollapsed] = useState(false)
  const openDisc = discrepancies.filter((d) => d.status === 'OPEN').length
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg-base)' }}>
      ...
      >
        <div className="h-14 flex items-center px-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold badge-chronic">⊕</div>
            {!collapsed && (
              <div>
                <h1 className="text-sm font-bold text-white">Chronicle</h1>
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5">
          {!collapsed && <p className="text-[9px] font-semibold uppercase tracking-widest mb-2 px-2" style={{ color: 'var(--text-muted)' }}>Coordinator</p>}
          {NAV.map(({ to, icon: Icon, label, badge, end }) => {
            const count = badge === 'exceptions' ? stats.openExceptions : badge === 'disc' ? openDisc : 0
            return (
              <NavLink
                key={to}
                to={to}
        ...
        <button type="button" onClick={() => setCollapsed((v) => !v)} className="absolute -right-3 top-16 w-6 h-6 rounded-full flex items-center justify-center z-10"
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2">
          {notifications.map((n) => (
            <div key={n.id} className="animate-slide-in px-4 py-3 rounded-lg text-sm font-medium text-white max-w-sm" style={{
